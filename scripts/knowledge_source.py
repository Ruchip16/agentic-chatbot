...
class Igloo:
    ...
    def get_children_from_parent(self, parent_path: str = None, parent_object_id: str = None, recursive: bool = False):
        ...
        if parent_path is not None:
            ...
            if response is None:
                raise ValueError(f"Parent path {parent_path} does not exist. Please check the path and try again.")
...
def fetchall(url_fragment: str, recursive: bool = False, attachments: bool = True, metadata: dict = {}, **kwargs):
    """
    ...
    fragment_documents = igloo.get_children_from_parent(parent_path=url_fragment, recursive=recursive)
    ...
    for document in fragment_documents:
        if document["isPublished"] and not document["IsArchived"]:
            ...
            file_metadata = {key: value for key, value in document.items() if key not in used_columns}
